...
export const useDishesStore = defineStore('dishes', {
  ...
  getters: {
    getDishes: (state):Dishe[] => state.dishes,
  },
  actions: {
    deleteDishe(disheId: number) {
      this.dishes = this.dishes.filter((dishe) => dishe.id !== disheId)
    },
    addDishe(dishe: Dishe) {
      let id = 1
      if (this.dishes.length) {
        id = Math.max(...this.dishes.map((dishe:Dishe) => dishe.id)) + 1
      }
      dishe.id = id
      this.dishes.push(dishe)
    },
    updateDishe(dishe: Dishe) {
      const index = this.dishes.findIndex((item:Dishe) => item.id === dishe.id);
      if (index !== -1) {
        Object.assign(this.dishes[index], dishe)
      }
    }
  },
  persist: {
